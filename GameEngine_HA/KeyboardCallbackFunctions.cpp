	...
	int action, int mods)
{
	if (!isTyping)
	{
		...
		switch (theEditMode)
		{
		case MOVING_PHYSICS_OBJECT:
		{
			if (key == GLFW_KEY_A && action == GLFW_PRESS)     // Left
			{
				g_UserInput.input |= INPUT_A;
			}
			if (key == GLFW_KEY_D && action == GLFW_PRESS)     // Right
			{
				g_UserInput.input |= INPUT_D;
			}
			if (key == GLFW_KEY_W && action == GLFW_PRESS)     // Forward
			{
				g_UserInput.input |= INPUT_W;
			}
			if (key == GLFW_KEY_S && action == GLFW_PRESS)     // Backwards
			{
				g_UserInput.input |= INPUT_S;
			}
			if (key == GLFW_KEY_SPACE && action == GLFW_PRESS)     // Backwards
			{
				g_UserInput.input |= INPUT_FIRE;
			}
			if (key == GLFW_KEY_R && action == GLFW_PRESS)     // Backwards
			{
				g_UserInput.input |= INPUT_R;
			}
		}
		break;
		case MOVING_CAMERA:
